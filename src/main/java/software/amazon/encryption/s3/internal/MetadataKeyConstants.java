...
public class MetadataKeyConstants {
    ...
    /**
     * Checks if multiple version-exclusive keys are present in metadata.
     */
    public static boolean hasExclusiveKeyCollision(Map<String, String> metadata) {
        boolean hasV1Key = metadata.containsKey(ENCRYPTED_DATA_KEY_V1);
        boolean hasV2Key = metadata.containsKey(ENCRYPTED_DATA_KEY_V2);
        boolean hasV3Key = metadata.containsKey(ENCRYPTED_DATA_KEY_V3);
        
        int exclusiveKeyCount = (hasV1Key ? 1 : 0) + (hasV2Key ? 1 : 0) + (hasV3Key ? 1 : 0);
        return exclusiveKeyCount > 1;
    }
